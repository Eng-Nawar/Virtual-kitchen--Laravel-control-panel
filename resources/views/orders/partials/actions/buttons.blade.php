...
<div class="card-footer py-4">
    ...
    @if(auth()->user()->hasRole('chief'))
      ...
      @elseif($lastStatusAlisas == "assigned_to_driver"&&$order->delivery_method.""!="2")
        <button type="button" class="btn btn-primary" onClick=(setSelectedOrderId({{ $order->id }}))  data-toggle="modal" data-target="#modal-asign-driver">{{ __('Prepared') }}</button>
      @else
        <p>{{ __('No actions for you right now!') }}</p>
     @endif
    @endif
    ...
    @if(auth()->user()->hasRole('driver'))
      ...
      @elseif($lastStatusAlisas == "prepared"&&$order->delivery_method.""!="2")
        <button type="button" class="btn btn-primary" onClick=(setSelectedOrderId({{ $order->id }}))  data-toggle="modal" data-target="#modal-asign-driver">{{ __('Picked up') }}</button>
      @else
        <p>{{ __('No actions for you right now!') }}</p>
     @endif
    @endif
